...
| 创建Python虚拟环境    | `uv venv --python 3.13.5`  | `poetry env use 3.13.5`        | `pyenv virtualenv 3.13.5 venv_name` | `uv` 默认不安装 `pip`，增加参数 `--seed` 可安装。`poetry` 会自动创建 |
| 在当前目录锁定Python版本 | `uv python pin 3.13.5`     | 不需要                            | `pyenv local venv_name`             | 目录下生成 `.python-version` ，`poetry` 不生成             |
| 激活python虚拟环境    | `uv run command`           | `poetry env use 3.13.5`        | `pyenv shell venv_name`             | `pyenv` 用过 `local` 后，cd到目录自动激活                    |
| 查看当前Python版本    | `uv run python --version`  | `poetry env info`              | `pyenv version`                     | 也可以 `poetry run python -V`                        |
